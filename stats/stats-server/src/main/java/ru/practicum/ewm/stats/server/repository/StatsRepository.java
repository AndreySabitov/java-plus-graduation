...
public interface StatsRepository extends JpaRepository<EndpointHit, Long> {
    ...
    @Query("select new ru.practicum.ewm.dto.stats.StatsDto(r.app, r.uri, count(distinct(r.ip))) " +
    ...
    @Query("select new ru.practicum.ewm.dto.stats.StatsDto(r.app, r.uri, count(r)) " +
            "from EndpointHit AS r " +
            "where r.createdDate >= :start AND r.createdDate <= :end AND uri in (:uris) " +
            "group by r.app, r.uri " +
